...
namespace Admin.ViewComponents
{
    public class ServicesListViewComponent : ViewComponent
    {
        ...
        private async Task<PaginatedList<Service>> GetItemsAsync(int isActive, int? pageNumber, int? pageSize, string textSearch)
        {
            ...
            var pagedList = PaginatedList<Service>.Create(result.AsQueryable(), pageNumber ?? 1, pageSize ?? 5);
